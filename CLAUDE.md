...
## The hand-out kit (`prime/kit.py`) — printable business card + flyer
The pieces a teacher prints and carries into an educational centre. Three public
pages, open to everyone on purpose (a staff gate would defeat the point — we *want*
these walking off the site):
- `/kit/card/` — the business card, **90 × 50 mm**, front and back at true size;
- `/kit/card/sheet/` (`?side=front|back`) — **ten cards on one A4** with cut guides;
- `/kit/flyer/` — the **A4 two-page flyer** for centres.
There is no PDF library: like every other print sheet on the site, the page *is* the
document and the visitor presses Ctrl+P → "Save as PDF" (A4, margins **None**,
"Background graphics" **on** — the toolbar says so).
**Where things live.** Words: `prime/kit.py` (`COPY['uz']` / `COPY['en']`). Paper:
`templates/kit/*`. Ink: the `kt-*` kit in `static/css/kit.css` — standalone, it does
...
Free makes a centre director suspicious; a shorter working day makes them curious.
The site's own older copy (the About stat bar, "Join Free") is a separate question
and was left alone. The **slogan on these pieces** is
«Kuchli fikrlar chegaralarni kengaytiradi» — the card sets it `nowrap` at a size
measured against the left column (48.9 mm), so a longer slogan does not wrap, it
slides under the QR silently. Measure it.
**The copy is bilingual data, not `{% trans %}`** — the Logic Arena's call: a
headline is content, not interface. `?lang=en|uz` switches the *piece* and is
deliberately independent of the visitor's own EN/UZ switch (the card is for the
centre director, not for the person at the keyboard). Default is **Uzbek**.
Only the About page's section around them is gettext.
**⚠️ THE PAPER GATE.** Both failure modes here are silent and expensive — a
hundred cards come off the printer wrong. Measure, do not eyeball:
- **Page one of the flyer must be ≤ 297 mm.** It shipped at 317 mm once, which
  prints as three pages. Check with `document.querySelector('.kt-flyer').scrollHeight`.
- **The card back has a 50 mm budget and clips.** At 52.9 mm the navy contact bar
  was cut in half and nothing warned. Every millimetre in `.kt-card--back` is spent
  against that budget.
- The real check is `--headless --print-to-pdf` in Chrome, then look at the pages:
  `"/Applications/Google Chrome.app/Contents/MacOS/Google Chrome" --headless
  --no-pdf-header-footer --print-to-pdf=out.pdf --virtual-time-budget=8000 <url>`.
  The card sheet must be **1** page, the flyer exactly **2**.
**The QR codes are committed files, not runtime work** — `static/images/qr/*.svg|png`,
regenerated by `python manage.py gen_qr` (dev-only, `pip install segno`, like
`gen_icons`). Error correction **H**, a white quiet zone, and **no logo in the
middle**. They point at `https://www.powerty.uz/` — **with the www**: the apex has no
HTTPS certificate and a printed QR cannot be corrected afterwards. Scan a new code
with a real phone before printing anything.
The About page previews both pieces in an iframe using `?bare=1` (no toolbar, no
stage), which is why the three views carry `@xframe_options_sameorigin` — the
project default is `X-Frame-Options: DENY`. A live preview beats a screenshot that
goes stale the moment a lesson is added.
Tests: `prime/tests.py` — the two languages' key sets must match, the QR files must
exist, the sheet must hold ten cards, the pages must stay public.
